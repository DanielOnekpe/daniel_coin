"""
The message creates a framework for messages
Makes message standardized throughout the network

Types of message : Peer, Block, Transaction, Ping
The message also creates this messages based on the type of message
"""

from marshmallow import Schema, fields, post_load
from marshmallow_oneofschema import OneOfSchema

from daniel_coin.schema import Peer, Block, Transaction, Ping
...
def meta(ip, port, version="daniel_coin_0.1"):
    return {
        "client": version,
...
def create_transaction_message(external_ip, external_port, transaction):
    return BaseSchema().dumps({
        "meta": meta(external_ip, external_port),
        "message": {"name": "transaction", "payload": transaction},
    })
