...
import structlog
from daniel_coin.messages import (
    create_peers_message,
    create_block_message,
    create_transaction_message,
    create_ping_message,
)
from daniel_coin.transactions import validate_transaction
...
class P2PProtocol:
    ...
    async def handle_ping(self, message, writer):
        """
        Executed when we recieve a ping message
        ...
        # If they are a miner, lets mark them as such
        writer.is_miner = message["payload"]["is_miner"]

        # Lets send our 20 most alive peers to this user
        ...
        # Lets send them blocks if they have less than us
    ...
    async def handle_transaction(self, message, writer):
        """
        Executed when we recieve a transaction that was broadcast by a peer
        :param message: The contents of the message
        :param writer: The writer of the message
        :return: None
        """
    ...
    async def handle_block(self, message, writer):
        ...
        # Give the block to the blockchain to append it valid
        ...
        for peer in self.connection_pool.get_alive_peers(20):
            await self.send_message(
                peer,
                create_block_message(self.server.external_ip, self.server.external_port, block),
            )
    ...
    async def handle_peers(self, message, writer):
        ...
        for peer in peers:
            # Create a connection and and them to our connection pool if successful
            reader, writer = await asyncio.open_connection(peer["ip"], peer["port"])

            # We are only interested in the writer
